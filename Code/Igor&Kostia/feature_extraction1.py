...
df_pro_desc = pd.read_csv('processing_text/df_product_descriptions_processed.csv')
df_attr_bullets = pd.read_csv('processing_text/df_attribute_bullets_processed.csv')
df_all = pd.read_csv('processing_text/df_train_and_test_processed.csv')
...
df_all[['id','above15_dummy_frequency_of_beforethekey_thekey','description20_percentile']].to_csv("features/df_feature_above15_ext.csv", index=False)
...
df_all.drop(string_variables_list,axis=1).to_csv("features/df_basic_features.csv", index=False)
...
material_dict={}
import csv
with open('processing_text/brand_statistics.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        brand_dict[row['name']]={'cnt_attribute': int(row['cnt_attribute']), 'cnt_query': int(row['cnt_query'])}
        
with open('processing_text/material_statistics.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
...
df_all.drop(string_variables_list,axis=1).to_csv("features/df_brand_material_dummies.csv", index=False)
print 'save file time:',round((time()-t0)/60,1) ,'minutes\n'
t0 = time()  
...
df_all.drop(string_variables_list,axis=1).to_csv("features/df_thekey_dummies.csv", index=False)
print 'save file time:',round((time()-t0)/60,1) ,'minutes\n'
t0 = time()         
